...
public class SingleLaplacianFeature extends AbstractFeatureOp {
		...
		List<RandomAccessibleInterval<FloatType>> output)
	{
		...
		Img<DoubleType> derivatives = ops().create().img(RevampUtils.appendDimensionToInterval(interval,
			0, n), new DoubleType());
		List<RandomAccessibleInterval<DoubleType>> d = RevampUtils.slices(derivatives);
		for (int i = 0; i < n; i++)
			differenciate(input, i, d.get(i));
		LoopBuilder.setImages(Views.collapse(derivatives), output.get(0)).forEachPixel((x, sum) -> sum
			.setReal(sum(x, n)));
	}
	...
	private void differenciate(RandomAccessible<FloatType> input, int d,
