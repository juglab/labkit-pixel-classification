...
public class GaborFeature {
	...
	public static Feature single(double sigma, double gamma, double psi, double frequency, int nAngles) {
		return new SingleGaborFeature(sigma, gamma, psi, frequency, nAngles, x -> {});
	}
	...
	public static Feature singleLegacy(double sigma, double gamma, double psi, double frequency, int nAngles) {
		return new SingleGaborFeature(sigma, gamma, psi, frequency, nAngles, GaborFeature::normalize);
	}
		...
		for(int i=0; i < 2; i++)
			for(double gamma = 1; gamma >= 0.25; gamma /= 2)
				for(int frequency = 2; frequency<3; frequency ++)
				{
					...
					features.add(new SingleGaborFeature(1.0, gamma, psi, frequency, nAngles, postProcessSlice ));
				}
		...
		for(int i=0; i < 2; i++)
			for(double sigma = 2.0; sigma <= 4.0; sigma *= 2)
				for(double gamma = 1.0; gamma <= 2.0; gamma *= 2)
					...
					{
						final double psi = Math.PI / 2 * i;
						features.add(new SingleGaborFeature(sigma, gamma, psi, frequency, nAngles, postProcessSlice ));
					}
		return features;
	}
	 ...
	 */
	private static class SingleGaborFeature implements Feature {

		private static final OpService ops = RevampUtils.ops();

		private final List<Img<FloatType>> kernels;

		private final String details;

		private final Consumer<RandomAccessibleInterval<FloatType>> postProcessSlice;
		...
		public SingleGaborFeature(double sigma, double gamma, double psi, double frequency, int nAngles, Consumer<RandomAccessibleInterval<FloatType>> postProcessSlice) {
			...
			details = "_" + sigma + "_" + gamma + "_" + (int) (psi / (Math.PI / 4)) + "_" + frequency;
		}
		...
		private static List<Img<FloatType>> initGaborKernels(double sigma, double gamma, double psi, double frequency, int nAngles) {
