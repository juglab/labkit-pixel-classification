...
public class MultiDimensionConvolution<T> implements Convolution<T> {
	 ...
	 * Constructor
	 *
	 * @param numDimensionToConvolution Function, when applied to a certain number
	 *          (e.g. 2), it must return a {@link Convolution} that can be applied
	 *          to images of that dimensions (e.g gauss 2d).
	 */
	...
	public Interval requiredSourceInterval(final Interval targetInterval) {
		return getCachedConvolution(targetInterval.numDimensions()).requiredSourceInterval(
			targetInterval);
	}
