...
public class CubicEquationTest {
	...
	public void testPrecision() {
		...
		for (int i = 0; i < 1_000_000; i++) {
			...
			double b = relativeError(expectedX, x);
			if(b > 1e-3)
				System.out.println(Arrays.toString(expectedX));
			relativeError = Math.max(relativeError, b);
		}
		System.out.println(relativeError);
		System.out.println(-log2(relativeError));
	}

	private double relativeError(double[] expectedX, double[] x) {
		double max_error = 0;
		for (int i = 0; i < 3; i++) {
